...
const getAgendamentos = (dataInicio, dataFim, codigoTransportadora, codigoProduto, codigoSafra, situacao, tipoAgendamento) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.all(query, params, (err, rows) => {
            if (err) {
                ...
                const formattedRows = rows.map(row => ({
                    ...row,
                    DataHoraEntrada: moment(row.DataHoraEntrada).format('DD/MM/YYYY HH:mm:ss'),
                    DataHoraSaida: moment(row.DataHoraSaida).format('DD/MM/YYYY HH:mm:ss')
                }));
                resolve(formattedRows);
            }
        });
