...
const listarPortarias = async (req, res) => {
    try {
        ...
        // Filtrar portarias se necessário
        const portariasFiltradas = portarias.filter(
            (portaria) => portaria.SituacaoPortaria !== "Indisponível" // Supondo que haja um campo para filtrar como no exemplo de agendamentos
        );

        if (portariasFiltradas.length === 0) {
            return res.status(204).send(); // Retornar 204 se não houver resultados
        }

        res.json(portariasFiltradas);
    } catch (error) {
...
const buscarPortariaPorAgendamento = async (req, res) => {
  try {
      ...
      if (portaria) {
          ...
          res.status(204).send(); // Retorna status 204 se não encontrar dados
      }
  } catch (error) {
      res.status(500).send({ message: "Erro ao buscar dados da portaria: " + error.message });
  }
